...
export class ClientUiStateHandlerService implements OnDestroy {
  ...
  private readonly isSpecificRemModeState$: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(false);
  public readonly isSpecificRemMode$: Observable<boolean> = this.isSpecificRemModeState$.asObservable();
  ...
  public readonly remSizePx$: Observable<number> = combineLatest([this.windowWidth$, this.isSpecificRemMode$]).pipe(
    map(([windowWidth, isSpecificRemMode]: [number, boolean]) => {
  ...
  public setIsSpecificRemMode(isSpecificRemMode: boolean): void {
    this.isSpecificRemModeState$.next(isSpecificRemMode);
  }
